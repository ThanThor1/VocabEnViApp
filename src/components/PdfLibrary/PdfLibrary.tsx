...
export default function PdfLibrary({ pdfs, selectedPdfId, onSelectPdf, onImport, onChange }: Props) {
  ...
  const handleDeletePermanent = async (pdfId: string) => {
    setConfirmPermanentDeleteId(pdfId)
  }
  return (
    <ErrorBoundary>
      {confirmPermanentDeleteId && (
        <ConfirmModal
          ...
          message="Permanently delete this PDF? This cannot be undone."
          confirmText="Delete"
          cancelText="Cancel"
          danger
          onCancel={() => setConfirmPermanentDeleteId(null)}
          onConfirm={async () => {
            const pdfId = confirmPermanentDeleteId
      ...
      <div className="p-6 border-b border-slate-200 dark:border-slate-700">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">PDF Library</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Your collection</p>
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-3">
                    <svg
                      className={`w-5 h-5 flex-shrink-0 ${
                        selectedPdfId === pdf.pdfId && mode === 'library' ? 'text-white' : 'text-slate-600 dark:text-slate-400'
                      }`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
