...
export default function ManagerPdf() {
  ...
  const [selectedPdfId, setSelectedPdfId] = usePersistedState<string | null>('managerPdf_selectedPdfId', null)
  ...
  // Restore last selected pdf after list loads
  useEffect(() => {
    if (restoredPdfRef.current) return
    if (!pdfs || pdfs.length === 0) return
    if (!selectedPdfId) return
    const found = (pdfs || []).find((x: any) => x && x.pdfId === selectedPdfId)
    if (!found) return
    restoredPdfRef.current = true
    selectPdf(found).catch(() => {})
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pdfs, selectedPdfId])
  ...
  async function selectPdf(p:any) {
    setSelectedPdf(p)
    setSelectedPdfId(p && p.pdfId ? String(p.pdfId) : null)
    if (p && p.deckCsvPath) {
      try {
  ...
  return (
    <ErrorBoundary>
      ...
      {/* Left Sidebar */}
      <div className="w-80 border-r border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm overflow-y-auto">
        <div className="p-6 border-b border-slate-200 dark:border-slate-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">PDF Decks</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">Manage vocabularies</p>
            </div>
          </div>
        </div>
              ...
              >
                <div className="flex items-center gap-3">
                  <svg className={`w-5 h-5 flex-shrink-0 ${
                    selectedPdf?.pdfId === p.pdfId ? 'text-white' : 'text-slate-600 dark:text-slate-400'
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className={`text-xs truncate ${
                      selectedPdf?.pdfId === p.pdfId ? 'text-violet-100' : 'text-slate-500 dark:text-slate-400'
                    }`}>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 py-4 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Manage PDF Vocabulary</h1>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">View and edit words from your PDF decks</p>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={loadPdfs}
          ...
          {selectedPdf ? (
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-6 mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-slate-900 dark:text-white">{selectedPdf.baseName}</h3>
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{selectedPdf.deckCsvPath}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <span className="px-2 py-1 bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 text-xs font-semibold rounded-full">
                          {rows.length} words
                        </span>
                      </div>
                    </div>
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <div className="inline-block p-6 bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 rounded-2xl mb-6">
                  <svg className="w-16 h-16 text-slate-400 dark:text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                </div>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">No PDF Selected</h3>
                <p className="text-slate-600 dark:text-slate-400">Select a PDF from the sidebar to view its vocabulary deck</p>
              </div>
            </div>
          )}
        </div>
